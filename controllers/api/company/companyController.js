...
const companyController = {
  ...
  onlinePage: async (req, res) => {
    try {
      // const result = await companyService.onlinePage();
      const { name, category, date_from } = req.query;
      let params = {};
      let events;
      if (name) {
        params.name = name;
        events = await Company.find({ companyName: name,onlinePay: true })
          .populate("owner")
          .populate("images")
      ...
      if (category) {
        ...
        events = await Company.find({ category: eventCats._id,onlinePay: true })
          .populate("owner")
          .populate("images")
          .populate("phoneNumbers")
          .populate("services")
      ...
      if (!category && !name) {
        events = await Company.find({onlinePay: true })
          .populate("owner")
          .populate("images")
          .populate("phoneNumbers")
          .populate("services")
      ...
      // const userDb = await User.findById(verify.id);
  ...
  addFavorites: async (req, res) => {
    try {
      ...
      //   const review= userDb.company_favorites.includes(companyId)
      //   if(review){
      //     res.status(300).send({message:"вы уже добавили"})
      //   }else{
      //     userDb.company_favorites.push(companyId)
      //     await userDb.save()
      //   }
  ...
  addCommets: async (req, res) => {
    try {
      ...
      if (!companyId || !text) {
      ...
      } else if(authHeader&&authHeader!=="null"){
        const token = authHeader.split(" ")[1];
        ...
        // Update the Company document to add the new comment
  ...
  like: async (req, res) => {
    try {
      ...
      if (!authHeader || !companyId) {
        ...
        if (!findLike) {
          // Create a new CompanyLikes document
          const newLike = new companyLikes({ userId, companyId });
          await newLike.save();

          // Update the Company document to add the new like
  ...
  near: async (req, res) => {
    try {
      ...
      const resultChanged1 = await Company.findOneAndUpdate(
        { _id: id },
        {
          $push: { view: companyView._id }, // Increment view count
  ...
  single: async (req, res) => {
    try {
      ...
      if (!ifView) {
        const newView=new companyView({userId:user.id,companyId:id,date:moment.tz(process.env.TZ).format()})
        await newView.save()
      }
      const companyView = new companyView({
        userId: user.id,
  ...
  //   try {
  //     const { companyId } = req.body;
  //     const result = await companyService.getOne(companyId);
  //     res.status(200).send(result);
  //   } catch (error) {
  //     console.error(error);
  //   }
  ...
  //   try {
  //     const result = await companyService.regisrtation();
  //   } catch (error) {
  //     console.error(error);
  //   }
  // },
